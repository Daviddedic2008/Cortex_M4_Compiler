...
Single pass compiler  \***2**\*, greedy register allocator  \***1**\*, backpatching for jumps, constant folding, lazy register storage, lazy flag storage.
...
You CANNOT assume that if the left side of an AND is false it won't evaluate the right.
